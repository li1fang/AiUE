from __future__ import annotations

import argparse
from pathlib import Path

from _bootstrap import ensure_aiue_paths

REPO_ROOT = ensure_aiue_paths()

from _demo_common import resolve_report_path
from _gate_common import (
    build_discussion_signal,
    default_latest_report_path,
    default_output_root,
    make_failed_requirement,
    now_utc,
    write_report_pair,
)

from aiue_core.report_writer import make_compatibility_block, with_report_envelope
from aiue_core.schema_utils import load_json, load_workspace_config
from aiue_unreal.host_bridge import run_host_auto_ue_cli

GATE_ID = "real_fx_item_kind_r2"
DEFAULT_D1_LATEST_NAME = "latest_demo_stage_d1_onboarding_report.json"
DEFAULT_FX_FIXTURE_ASSET = "/Niagara/DefaultAssets/Templates/Systems/FountainLightweight.FountainLightweight"
DEFAULT_FX_SLOT_NAME = "fx"
DEFAULT_FX_ATTACH_SOCKET = "WeaponSocket"
DEFAULT_TRACKED_FX_MIN_COVERAGE = 0.0001


def parse_args():
    parser = argparse.ArgumentParser(description="Run the AiUE R2 real FX item kind gate.")
    parser.add_argument("--workspace-config", required=True)
    parser.add_argument("--d1-report-path")
    parser.add_argument("--output-root")
    parser.add_argument("--latest-report-path")
    parser.add_argument("--fx-asset-path", default=DEFAULT_FX_FIXTURE_ASSET)
    parser.add_argument("--fx-slot-name", default=DEFAULT_FX_SLOT_NAME)
    parser.add_argument("--attach-socket-name", default=DEFAULT_FX_ATTACH_SOCKET)
    parser.add_argument("--tracked-fx-min-coverage", type=float, default=DEFAULT_TRACKED_FX_MIN_COVERAGE)
    return parser.parse_args()


def default_latest_d1_report_path(workspace: dict) -> Path:
    return Path(workspace["paths"]["aiue_repo_root"]).expanduser().resolve() / "Saved" / "verification" / DEFAULT_D1_LATEST_NAME


def normalize_asset_path(asset_path: str | None) -> str:
    text = str(asset_path or "").strip()
    if not text:
        return ""
    if "." not in text:
        return text
    package_path, object_name = text.rsplit(".", 1)
    leaf_name = package_path.rsplit("/", 1)[-1]
    if object_name == leaf_name:
        return package_path
    return text


def ready_demo_packages(d1_report: dict) -> list[dict]:
    results = []
    for entry in ((((d1_report.get("scene_sweep") or {}).get("result") or {}).get("package_results") or [])):
        if entry.get("status") != "pass":
            continue
        if not entry.get("package_id") or not entry.get("host_blueprint_asset_path"):
            continue
        results.append(
            {
                "package_id": str(entry.get("package_id") or ""),
                "sample_id": str(entry.get("sample_id") or ""),
                "host_blueprint_asset_path": str(entry.get("host_blueprint_asset_path") or ""),
            }
        )
    return sorted(results, key=lambda item: item["package_id"])


def fx_binding(slot_name: str, attach_socket_name: str, fx_asset_path: str) -> dict:
    return {
        "slot_name": str(slot_name or DEFAULT_FX_SLOT_NAME),
        "item_package_id": "r2_default_niagara_fixture",
        "item_kind": "niagara_system",
        "attach_socket_name": str(attach_socket_name or DEFAULT_FX_ATTACH_SOCKET),
        "niagara_system_asset": str(fx_asset_path or DEFAULT_FX_FIXTURE_ASSET),
        "consumer_ready": True,
    }


def inspect_runtime(workspace: dict, output_root: Path, host_key: str, package: dict, level_path: str, binding: dict) -> tuple[dict, Path]:
    result_path = output_root / "runtime_checks" / f"{package['package_id']}.json"
    result_path.parent.mkdir(parents=True, exist_ok=True)
    try:
        host_payload = run_host_auto_ue_cli(
            workspace_or_config=workspace,
            mode="editor_rendered",
            command="inspect-slot-runtime",
            params={
                "package_id": package["package_id"],
                "sample_id": package["sample_id"],
                "host_blueprint_asset_path": package["host_blueprint_asset_path"],
                "level_path": level_path,
                "required_slots": ["weapon", binding["slot_name"]],
                "slot_binding_overrides": [binding],
            },
            output_path=str(result_path.resolve()),
            host_key=host_key,
        )
        return dict((host_payload.get("payload") or {}).get("result") or {}), result_path
    except Exception:
        if result_path.exists():
            return dict((load_json(result_path).get("result") or {})), result_path
        raise


def inspect_visual(workspace: dict, output_root: Path, host_key: str, package: dict, level_path: str, binding: dict) -> tuple[dict, Path]:
    result_path = output_root / "visual_checks" / f"{package['package_id']}.json"
    capture_root = output_root / "visual_checks" / package["package_id"] / "captures"
    result_path.parent.mkdir(parents=True, exist_ok=True)
    capture_root.mkdir(parents=True, exist_ok=True)
    try:
        host_payload = run_host_auto_ue_cli(
            workspace_or_config=workspace,
            mode="editor_rendered",
            command="inspect-host-visual",
            params={
                "package_id": package["package_id"],
                "sample_id": package["sample_id"],
                "host_blueprint_asset_path": package["host_blueprint_asset_path"],
                "level_path": level_path,
                "output_root": str(capture_root.resolve()),
                "slot_binding_overrides": [binding],
                "tracked_slots": [binding["slot_name"]],
            },
            output_path=str(result_path.resolve()),
            host_key=host_key,
        )
        return dict((host_payload.get("payload") or {}).get("result") or {}), result_path
    except Exception:
        if result_path.exists():
            return dict((load_json(result_path).get("result") or {})), result_path
        raise


def evaluate_runtime_result(result: dict, result_path: Path, binding: dict) -> tuple[dict, list[dict]]:
    failed_requirements = []
    slot_name = binding["slot_name"]
    expected_asset_path = normalize_asset_path(binding["niagara_system_asset"])
    slot_bindings = list(result.get("slot_bindings") or [])
    managed_components_by_slot = dict(result.get("managed_components_by_slot") or {})
    slot_attach_state = list(result.get("slot_attach_state") or [])
    fx_binding_result = next((entry for entry in slot_bindings if entry.get("slot_name") == slot_name), {})
    fx_component = dict(managed_components_by_slot.get(slot_name) or {})
    fx_attach_state = next((entry for entry in slot_attach_state if entry.get("slot_name") == slot_name), {})

    if result.get("status") != "pass":
        failed_requirements.append(
            make_failed_requirement(
                "r2_runtime_failed",
                "The real FX slot runtime inspection did not pass.",
                result_path=str(result_path.resolve()),
                errors=list(result.get("errors") or []),
            )
        )
    if fx_binding_result.get("item_kind") != "niagara_system":
        failed_requirements.append(
            make_failed_requirement(
                "r2_fx_wrong_item_kind",
                "The FX slot did not resolve to niagara_system.",
                result_path=str(result_path.resolve()),
                binding=fx_binding_result,
            )
        )
    if normalize_asset_path(fx_binding_result.get("asset_path")) != expected_asset_path:
        failed_requirements.append(
            make_failed_requirement(
                "r2_fx_binding_asset_mismatch",
                "The FX slot binding did not resolve to the expected Niagara system asset.",
                result_path=str(result_path.resolve()),
                expected_asset_path=expected_asset_path,
                actual_asset_path=fx_binding_result.get("asset_path"),
            )
        )
    if fx_component.get("class_name") != "NiagaraComponent":
        failed_requirements.append(
            make_failed_requirement(
                "r2_fx_component_missing",
                "The FX slot did not create a Niagara component.",
                result_path=str(result_path.resolve()),
                managed_component=fx_component,
            )
        )
    if not bool((fx_component.get("bounds") or {}).get("non_zero")):
        failed_requirements.append(
            make_failed_requirement(
                "r2_fx_bounds_invalid",
                "The Niagara FX slot component bounds are not valid.",
                result_path=str(result_path.resolve()),
                managed_component=fx_component,
            )
        )
    if fx_attach_state and fx_attach_state.get("resolved_attach_socket_exists") is False:
        failed_requirements.append(
            make_failed_requirement(
                "r2_fx_attach_unresolved",
                "The Niagara FX slot could not resolve an attach target on the PMX host.",
                result_path=str(result_path.resolve()),
                attach_state=fx_attach_state,
            )
        )

    return (
        {
            "package_id": result.get("package_id"),
            "sample_id": result.get("sample_id"),
            "status": "pass" if not failed_requirements else "fail",
            "host_blueprint_asset": result.get("host_blueprint_asset"),
            "fx_binding": fx_binding_result,
            "fx_component": fx_component,
            "fx_attach_state": fx_attach_state,
            "slot_conflicts": list(result.get("slot_conflicts") or []),
            "artifacts": {
                "result_path": str(result_path.resolve()),
            },
        },
        failed_requirements,
    )


def evaluate_visual_result(result: dict, result_path: Path, binding: dict, tracked_min_coverage: float) -> tuple[dict, list[dict]]:
    failed_requirements = []
    slot_name = binding["slot_name"]
    expected_asset_path = normalize_asset_path(binding["niagara_system_asset"])
    managed_components_by_slot = dict(result.get("managed_components_by_slot") or {})
    slot_attach_state = list(result.get("slot_attach_state") or [])
    fx_component = dict(managed_components_by_slot.get(slot_name) or {})
    fx_attach_state = next((entry for entry in slot_attach_state if entry.get("slot_name") == slot_name), {})
    shots = list(result.get("shots") or [])
    passing_visual_shots = sum(1 for shot in shots if shot.get("status") == "pass")
    passing_fx_shots = sum(
        1
        for shot in shots
        if Path(str(shot.get("image_path") or "")).exists()
        and float((((shot.get("tracked_slot_coverages") or {}).get(slot_name) or {}).get("coverage_ratio") or 0.0)) >= tracked_min_coverage
    )

    if result.get("status") != "pass":
        failed_requirements.append(
            make_failed_requirement(
                "r2_visual_failed",
                "The Niagara FX visual proof did not pass.",
                result_path=str(result_path.resolve()),
                errors=list(result.get("errors") or []),
            )
        )
    if normalize_asset_path(fx_component.get("asset_path")) != expected_asset_path:
        failed_requirements.append(
            make_failed_requirement(
                "r2_visual_fx_asset_mismatch",
                "The visual proof did not keep the expected Niagara system asset on the managed FX component.",
                result_path=str(result_path.resolve()),
                expected_asset_path=expected_asset_path,
                actual_asset_path=fx_component.get("asset_path"),
            )
        )
    if fx_component.get("class_name") != "NiagaraComponent":
        failed_requirements.append(
            make_failed_requirement(
                "r2_visual_fx_component_wrong_class",
                "The visual proof did not keep a Niagara component on the FX slot.",
                result_path=str(result_path.resolve()),
                managed_component=fx_component,
            )
        )
    if not bool((fx_component.get("bounds") or {}).get("non_zero")):
        failed_requirements.append(
            make_failed_requirement(
                "r2_visual_fx_bounds_invalid",
                "The visual proof did not keep valid Niagara FX bounds.",
                result_path=str(result_path.resolve()),
                managed_component=fx_component,
            )
        )
    if fx_attach_state and fx_attach_state.get("resolved_attach_socket_exists") is False:
        failed_requirements.append(
            make_failed_requirement(
                "r2_visual_fx_attach_unresolved",
                "The visual proof shows an unresolved Niagara FX attach state.",
                result_path=str(result_path.resolve()),
                attach_state=fx_attach_state,
            )
        )
    if passing_visual_shots <= 0:
        failed_requirements.append(
            make_failed_requirement(
                "r2_visual_no_passing_shots",
                "The Niagara FX proof did not produce any passing visual shots.",
                result_path=str(result_path.resolve()),
            )
        )
    if passing_fx_shots <= 0:
        failed_requirements.append(
            make_failed_requirement(
                "r2_fx_not_visible_in_shots",
                "The Niagara FX slot did not register enough on-screen coverage in any tracked visual shot.",
                result_path=str(result_path.resolve()),
                tracked_slot=slot_name,
                tracked_min_coverage=tracked_min_coverage,
            )
        )

    return (
        {
            "package_id": result.get("package_id"),
            "sample_id": result.get("sample_id"),
            "status": "pass" if not failed_requirements else "fail",
            "host_blueprint_asset": result.get("host_blueprint_asset"),
            "fx_component": fx_component,
            "fx_attach_state": fx_attach_state,
            "passing_visual_shots": passing_visual_shots,
            "passing_fx_shots": passing_fx_shots,
            "shots": shots,
            "artifacts": {
                "result_path": str(result_path.resolve()),
            },
        },
        failed_requirements,
    )


def main():
    args = parse_args()
    workspace = load_workspace_config(args.workspace_config)
    output_root = Path(args.output_root).expanduser().resolve() if args.output_root else default_output_root(workspace, REPO_ROOT, GATE_ID)
    latest_report_path = Path(args.latest_report_path).expanduser().resolve() if args.latest_report_path else default_latest_report_path(workspace, REPO_ROOT, GATE_ID)
    output_root.mkdir(parents=True, exist_ok=True)
    previous_report = load_json(latest_report_path) if latest_report_path.exists() else None
    previous_report_path = latest_report_path if latest_report_path.exists() else None

    failed_requirements: list[dict] = []
    d1_report_path = resolve_report_path(
        args.d1_report_path,
        default_latest_d1_report_path(workspace),
        "No latest_demo_stage_d1_onboarding_report.json could be resolved for R2.",
    )
    d1_report = load_json(d1_report_path)
    host_key = str(d1_report.get("host_key") or "demo")
    level_path = str(d1_report.get("level_path") or "/Game/Levels/DefaultLevel")
    packages = ready_demo_packages(d1_report)
    if len(packages) != 2:
        failed_requirements.append(
            make_failed_requirement(
                "r2_ready_package_count",
                "R2 requires exactly two ready demo packages from D1 onboarding.",
                expected=2,
                actual=len(packages),
                d1_report_path=str(d1_report_path.resolve()),
            )
        )

    binding = fx_binding(args.fx_slot_name, args.attach_socket_name, args.fx_asset_path)

    runtime_checks = []
    visual_checks = []
    for package in packages:
        try:
            runtime_result, runtime_result_path = inspect_runtime(workspace, output_root, host_key, package, level_path, binding)
            runtime_payload, runtime_failures = evaluate_runtime_result(runtime_result, runtime_result_path, binding)
            runtime_checks.append(runtime_payload)
            failed_requirements.extend(runtime_failures)

            visual_result, visual_result_path = inspect_visual(workspace, output_root, host_key, package, level_path, binding)
            visual_payload, visual_failures = evaluate_visual_result(visual_result, visual_result_path, binding, args.tracked_fx_min_coverage)
            visual_checks.append(visual_payload)
            failed_requirements.extend(visual_failures)
        except Exception as exc:
            failed_requirements.append(
                make_failed_requirement(
                    "r2_package_execution_failed",
                    "R2 failed while validating a ready bundle with a Niagara FX fixture.",
                    package_id=package["package_id"],
                    error=str(exc),
                )
            )

    status = "pass" if not failed_requirements else "fail"
    report = with_report_envelope(
        {
            "generated_at_utc": now_utc(),
            "gate_id": GATE_ID,
            "status": status,
            "success": status == "pass",
            "workspace_config": str(Path(args.workspace_config).expanduser().resolve()),
            "host_key": host_key,
            "level_path": level_path,
            "fixed_execution_profile": {
                "host_key": host_key,
                "mode": "editor_rendered",
                "fx_slot_name": binding["slot_name"],
                "fx_item_kind": binding["item_kind"],
                "fx_asset_path": binding["niagara_system_asset"],
                "attach_socket_name": binding["attach_socket_name"],
                "tracked_fx_min_coverage": float(args.tracked_fx_min_coverage),
            },
            "counts": {
                "required_ready_packages": 2,
                "resolved_ready_packages": len(packages),
                "runtime_checks": len(runtime_checks),
                "passing_runtime_checks": sum(1 for item in runtime_checks if item.get("status") == "pass"),
                "visual_checks": len(visual_checks),
                "passing_visual_checks": sum(1 for item in visual_checks if item.get("status") == "pass"),
                "passing_fx_visual_shots": sum(int(item.get("passing_fx_shots") or 0) for item in visual_checks),
            },
            "failed_requirements": failed_requirements,
            "resolved_package_ids": [package["package_id"] for package in packages],
            "runtime_checks": runtime_checks,
            "visual_checks": visual_checks,
            "discussion_signal": build_discussion_signal(
                status,
                failed_requirements,
                previous_report,
                previous_report_path=previous_report_path,
                first_pass_reason="r2_first_complete_pass",
            ),
            "artifacts": {
                "run_root": str(output_root.resolve()),
                "d1_report_path": str(d1_report_path.resolve()),
            },
        },
        schema_family="aiue_real_fx_item_kind_report",
        compatibility=make_compatibility_block(
            schema_family="aiue_real_fx_item_kind_report",
            notes=[
                "internal_r2_gate",
                "real_fx_item_kind",
                "niagara_system_runtime_path",
            ],
        ),
    )
    write_report_pair(report, output_root / f"{GATE_ID}.json", latest_report_path)
    return 0 if status == "pass" else 1


if __name__ == "__main__":
    raise SystemExit(main())
