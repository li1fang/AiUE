...
def parse_args():
    parser = argparse.ArgumentParser(description="Run the AiUE P3 FX vertical slice gate.")
...
def fx_binding(slot_name: str, attach_socket_name: str, fx_asset_path: str) -> dict:
    return {
        ...
        "item_package_id": "p3_fx_cylinder_fixture",
...
def evaluate_runtime_result(result: dict, result_path: Path, binding: dict) -> tuple[dict, list[dict]]:
    ...
    expected_asset_path = normalize_asset_path(binding["static_mesh_asset"])
    ...
    if result.get("status") != "pass":
        failed_requirements.append(
            make_failed_requirement(
                "p3_runtime_failed",
                "The FX slot runtime inspection did not pass.",
                result_path=str(result_path.resolve()),
                errors=list(result.get("errors") or []),
            )
        )
    if fx_binding_result.get("item_kind") != "static_mesh":
        failed_requirements.append(
            make_failed_requirement(
                "p3_fx_wrong_item_kind",
                "The FX slot did not resolve to static_mesh.",
                result_path=str(result_path.resolve()),
    ...
    if normalize_asset_path(fx_binding_result.get("asset_path")) != expected_asset_path:
        failed_requirements.append(
            make_failed_requirement(
                "p3_fx_binding_asset_mismatch",
                "The FX slot binding did not resolve to the expected FX proxy asset.",
                result_path=str(result_path.resolve()),
                expected_asset_path=expected_asset_path,
                actual_asset_path=fx_binding_result.get("asset_path"),
            )
        )
    if fx_component.get("class_name") != "StaticMeshComponent":
        failed_requirements.append(
            make_failed_requirement(
                "p3_fx_component_missing",
                "The FX slot did not create a static mesh component.",
                result_path=str(result_path.resolve()),
    ...
    if not bool((fx_component.get("bounds") or {}).get("non_zero")):
        failed_requirements.append(
            make_failed_requirement(
                "p3_fx_bounds_invalid",
                "The FX slot component bounds are not valid.",
                result_path=str(result_path.resolve()),
                managed_component=fx_component,
            )
        )
    if fx_attach_state and fx_attach_state.get("resolved_attach_socket_exists") is False:
        failed_requirements.append(
            make_failed_requirement(
                "p3_fx_attach_unresolved",
                "The FX slot could not resolve an attach target on the PMX host.",
                result_path=str(result_path.resolve()),
...
def evaluate_visual_result(result: dict, result_path: Path, binding: dict, tracked_min_coverage: float) -> tuple[dict, list[dict]]:
    ...
    if result.get("status") != "pass":
        failed_requirements.append(
            make_failed_requirement(
                "p3_visual_failed",
                "The FX composition visual proof did not pass.",
                result_path=str(result_path.resolve()),
                errors=list(result.get("errors") or []),
            )
        )
    ...
    if not bool((fx_component.get("bounds") or {}).get("non_zero")):
        failed_requirements.append(
            make_failed_requirement(
                "p3_visual_fx_bounds_invalid",
                "The visual proof did not keep a valid FX component bounds payload.",
                result_path=str(result_path.resolve()),
                managed_component=fx_component,
            )
        )
    if fx_attach_state and fx_attach_state.get("resolved_attach_socket_exists") is False:
        failed_requirements.append(
            make_failed_requirement(
                "p3_visual_fx_attach_unresolved",
                "The visual proof shows an unresolved FX attach state.",
                result_path=str(result_path.resolve()),
                attach_state=fx_attach_state,
            )
        )
    if passing_visual_shots <= 0:
        failed_requirements.append(
            make_failed_requirement(
                "p3_visual_no_passing_shots",
                "The FX composition proof did not produce any passing visual shots.",
                result_path=str(result_path.resolve()),
            )
        )
    if passing_fx_shots <= 0:
        failed_requirements.append(
            make_failed_requirement(
                "p3_fx_not_visible_in_shots",
                "The FX slot did not register enough on-screen coverage in any tracked visual shot.",
                result_path=str(result_path.resolve()),
...
def main():
    ...
    d1_report_path = resolve_report_path(
        ...
        "No latest_demo_stage_d1_onboarding_report.json could be resolved for P3.",
    )
    ...
    if len(packages) != 2:
        failed_requirements.append(
            make_failed_requirement(
                "ready_package_count",
                "P3 requires exactly two ready demo packages from D1 onboarding.",
                expected=2,
                actual=len(packages),
    ...
    if not failed_requirements:
        for package in packages:
            try:
                ...
                visual_evaluated, visual_failures = evaluate_visual_result(visual_raw, visual_path, binding, args.tracked_fx_min_coverage)
                visual_checks.append(visual_evaluated)
                failed_requirements.extend(visual_failures)
            except Exception as exc:
                failed_requirements.append(
                    make_failed_requirement(
                        "p3_package_execution_failed",
                        "P3 failed while validating a ready bundle with the FX slot fixture.",
    ...
    status = "pass" if not failed_requirements else "fail"
    discussion_signal = build_discussion_signal(status, failed_requirements, previous_report, previous_report_path, "p3_first_complete_pass")
    counts = {
    ...
    report_payload = with_report_envelope(
        {
            ...
            "fixed_execution_profile": {
                ...
                "fx_asset_path": binding["static_mesh_asset"],
                "tracked_fx_min_coverage": float(args.tracked_fx_min_coverage),
                "fx_fixture_kind": "static_mesh_proxy",
            },
            "counts": counts,
        ...
        schema_family="aiue_fx_vertical_slice_report",
        workflow_pack="pmx_pipeline",
        compatibility=make_compatibility_block(
            "aiue_fx_vertical_slice_report",
            notes=["internal_p3_gate", "generic_slot_runtime_fx_slice", "static_mesh_fx_proxy"],
        ),
    )
    report_path = output_root / "p3_fx_vertical_slice_report.json"
